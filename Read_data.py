...
def read_data(folder, stride=50, i_mix=True, i_filter=True, fs=64, cutoff=20.0, order=3, mode="pre"):
    ...
    for file_name in tqdm.tqdm(txt_name, desc='Processing files'):
        ...
        columns = ['Time', 'Ankle_hf', 'Ankle_v', 'Ankle_hl', 'Upper_leg_hf', 'Upper_leg_v', 'Upper_leg_hl', 'Trunk_hf', 'Trunk_v', 'Trunk_hl', 'label']
        df = pd.read_csv(file_path, sep=' ', header=None, names=columns)
        if i_filter:
            for col in ['Ankle_hf', 'Ankle_v', 'Ankle_hl', 'Upper_leg_hf', 'Upper_leg_v', 'Upper_leg_hl', 'Trunk_hf', 'Trunk_v', 'Trunk_hl']:
        ...
        while index + stride <= total_length:
            ...
            windowx = window.loc[:, ['Ankle_hf', 'Ankle_v', 'Ankle_hl', 'Upper_leg_hf', 'Upper_leg_v', 'Upper_leg_hl', 'Trunk_hf', 'Trunk_v', 'Trunk_hl']].reset_index(drop=True)
    ...
    test_indices = [i for i in remaining_indices if i not in validation_indices]
...
class Daphnetwindow(Dataset):
    def __init__(self, set, mode = "pre"):
        super().__init__()
        self.set = set
        self.mode = mode
    def __getitem__(self, index):
        if self.mode == "pre":
            ...
            x = torch.tensor(data_x, dtype=torch.float32).mT
